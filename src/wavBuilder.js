...
function wavBuilder(source, customHeader = {}) {
    ...
    if (bitType == 4) {
        for (var i = 0, j = source.length; i < j; i++) {
            set(source[i], 2, { reverse: true });
        }
    }
    else {
        set(source, null, { force: true });
    }

    return new Blob([].concat(data), { 'type': 'audio/wav' });

    function get(key) {
        return Object.hasOwnProperty(customHeader, key) ? customHeader[key] : header[key];
    }
    function set(d, len = d.length, { force = false, reverse = false } = {}) {
        if (Array.isArray(d) || force) {
